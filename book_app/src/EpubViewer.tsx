...
const EpubViewer = ({ url }) => {
  ...
  const goToLocation = (location) => {
    console.log('came in location ' , location.split('.xhtml')[0] , renditionRef.current)
    if (renditionRef.current) {
  ...
  const removeHighlight = (cfiRange) => {
    setHighlights((prev) => prev.filter((highlight) => highlight.cfiRange !== cfiRange));
    renditionRef.current.annotations.remove(cfiRange, 'highlight');
  };

  return (
    <div className="flex flex-col items-center p-4">
      <div ref={viewerRef} className="w-full h-80 bg-gray-100"></div>
      <div className="flex space-x-4 mt-4">
        <button onClick={goPrev} className="px-4 py-2 bg-blue-500 text-white rounded">Previous</button>
        <button onClick={goNext} className="px-4 py-2 bg-blue-500 text-white rounded">Next</button>
        <button onClick={() => goToLocation('epubcfi(/6/2[cover]!/6)')} className="px-4 py-2 bg-blue-500 text-white rounded">Go to Cover</button>
        <button onClick={addBookmark} className="px-4 py-2 bg-blue-500 text-white rounded">Add Bookmark</button>
      </div>
      ...
      <div className="mt-4 text-black">
        ...
        <ul>
          {Array.isArray(toc) && toc.map((chapter, index) => (
            <li key={index}>
              <button onClick={() => goToLocation(chapter.href)} className="text-blue-500 underline">
                {chapter.label}
              </button>
            </li>
          ))}
      ...
      <div className="mt-4 text-black">
        ...
        <ul>
          {bookmarks.map((cfi, index) => (
            <li key={index}>
              <button onClick={() => goToBookmark(cfi)} className="text-blue-500 underline">
                {cfi}
              </button>
            </li>
          ))}
        ...
        <ul>
          {highlights.map((highlight, index) => (
            <li key={index}>
              ...
              <button onClick={() => removeHighlight(highlight.cfiRange)} className="ml-2 text-red-500 underline">
                Remove
              </button>
            </li>
          ))}
